var ap5Plugin = {
	...
	updatePackTable: function(selectObject = undefined) {
		let packId = 0;
		if (typeof(selectObject) !== 'undefined') {
			selectObject = selectObject[0];
			let parentSelectObject = $(selectObject).closest('.product-variants.ap5-attributes');
			packId = parseInt(parentSelectObject.attr('data-id-product-pack'));
		}
		...
		$('.ap5-attributes').each(function (index, element) {
			...
			$(document).trigger("updatedProduct");
		});
		// Get quantity for each product
		$('.ap5-quantity-wanted').each(function (index, element) {
			id_product_pack = $(this).attr('data-id-product-pack');
		...
		$.ajax({
			...
			cache: false,
			success: function(jsonData, textStatus, jqXHR) {
				// The dates of each pack item has to be saved so that they can be reset after ajax calls
				packItemsDates = [];
				$('.dd_available .dd-info-container .dd-estimated-date').each(function() {
					packItemsDates.push($(this).html());
				});
				...
				if (window.customUpdatePackDeliveryDates) {
					customUpdatePackDeliveryDates(jsonData.packAttributesList, packId);
				}
				setTimeout(function(){
