...
jQuery(document).ready(function ($) {
	...
	if ($('.product-information').length > 0 && $('.advanceddateofdelivery_container').length > 0 && $('.social-sharing').length > 0) {
		$('.product-information').append($('.product-additional-info .social-sharing'));
	}
...
function refreshFooterProductDateOfDelivery()
{
	...
	$.each($('.adod_product_page_txt'), function()
	{
		...
		let replacementTextMini = 'Aucune date à afficher';
		let replacementTextMaxi = 'Aucune date à afficher';
		if (minimal_date != null) {
			replacementTextMini = minimal_date[0];
		}
		if (maximal_date != null) {
			replacementTextMaxi = maximal_date[0];
		}

		if (replacementTextMini == 'Aucune date à afficher' || replacementTextMaxi == 'Aucune date à afficher') {
			$('.advanceddateofdelivery_container').remove();
		}
		else {
			adod_product_page_txt_value = adod_product_page_txt_value.replace('%MINIMAL_DATE%', replacementTextMini);
			adod_product_page_txt_value = adod_product_page_txt_value.replace('%MAXIMAL_DATE%', replacementTextMaxi);
			adod_product_page_txt_value = adod_product_page_txt_value.replace('%NB_TOTAL_CARRIERS%', carrier_delivery_list);
			$(this).html(adod_product_page_txt_value);
