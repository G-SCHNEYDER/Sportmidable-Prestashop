...
{foreach from=$status key=k item=v}
	...
	{assign var=statusdate value=$v.date}
{/foreach}

	<tr class="customized customized-{$product['id_order_detail']|intval} product-line-row {if $status|@count > 0}status-{$statusid}{/if}">
		<td>
		...
		<td>
			...
			{if $can_edit}
				<span class='customtext'>{$product['customtext']}</span>
				<div class="product_price_edit" style="display:none;">
					<input type='text' name="customtext" value="{$product['customtext']}" />
				</div>
			{/if}
		</td>
		<td>
		...
		{foreach $customizationPerAddress as $customizationId => $customization}
			<tr class="customized customized-{$product['id_order_detail']|intval}">
				...
				<td class="cancelCheck standard_refund_fields current-edit" style="display:none">
					...
					{if ((Configuration::get('PS_ORDER_RETURN')) AND (int)($customization['quantity_returned']) < (int)($customization['quantity']))}
